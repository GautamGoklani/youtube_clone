...
export const getHomePageVideos = createAsyncThunk(
  ...
  // Asynchronous function to fetch homepage videos with optional pagination
  async (isNext: boolean, { getState }) => {
    ...
    const {
      ...
      `${YOUTUBE_API_URL}/search?maxResults=20&q="most popular videos worldwide by views"&key=${API_KEY}&part=snippet&type=video&${isNext ? `pageToken=${nextPageTokenFromState}` : ""
      }`
    );
