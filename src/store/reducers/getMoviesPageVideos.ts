...
import { MoviesPageVideos } from "../../Types";
...
export const getMoviesPageVideos = createAsyncThunk(
  // Defining the action type string for the asynchronous operation
  "youtubeApp/moviesPageVideos",

  // Asynchronous function to fetch movies videos with optional pagination
  async (isNext: boolean, { getState }) => {
    ...
    const {
      ...
      `${YOUTUBE_API_URL}/search?maxResults=20&q="Trending movies worldwide by views"&key=${API_KEY}&part=snippet&type=video&${isNext ? `pageToken=${nextPageTokenFromState}` : ""
      }`
    );
    ...
    const parsedData: MoviesPageVideos[] = await parseData(items);
