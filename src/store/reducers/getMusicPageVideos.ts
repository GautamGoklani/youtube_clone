...
import { MusicPageVideos } from "../../Types";
...
export const getMusicPageVideos = createAsyncThunk(
  // Defining the action type string for the asynchronous operation
  "youtubeApp/musicPageVidoes",

  // Asynchronous function to fetch music videos with optional pagination
  async (isNext: boolean, { getState }) => {
    ...
    const {
      ...
      `${YOUTUBE_API_URL}/search?maxResults=20&q="Trending Music Videos Worldwide by views"&key=${API_KEY}&part=snippet&type=video&${isNext ? `pageToken=${nextPageTokenFromState}` : ""
      }`
    );
    ...
    const parsedData: MusicPageVideos[] = await parseData(items);
