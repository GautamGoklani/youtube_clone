...
import { SportsPageVideos } from "../../Types";
...
export const getSportsPageVideos = createAsyncThunk(
  // Defining the action type string for the asynchronous operation
  "youtubeApp/sportsPageVideos",

  // Asynchronous function to fetch sports videos with optional pagination
  async (isNext: boolean, { getState }) => {
    ...
    const {
      ...
      `${YOUTUBE_API_URL}/search?maxResults=20&q="Trending sports videos worldwide by views"&key=${API_KEY}&part=snippet&type=video&${isNext ? `pageToken=${nextPageTokenFromState}` : ""
      }`
    );
    ...
    const parsedData: SportsPageVideos[] = await parseData(items);
